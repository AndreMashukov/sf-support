...
from app.support_bus.events import (
    ask_completed_payload,
    new_write_id,
    ticket_updated_payload,
)
...
def handle_command(event: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if command_type == "CreateTicket":
        return _handle_create_ticket(event)
    if command_type == "AppendMessage":
        return _handle_append_message(event)
...
def _handle_create_ticket(event: dict[str, Any]) -> dict[str, Any]:
    payload = event.get("payload") if isinstance(event.get("payload"), dict) else {}
...
def _handle_append_message(event: dict[str, Any]) -> dict[str, Any] | None:
    payload = event.get("payload") if isinstance(event.get("payload"), dict) else {}
    ...
    ticket = persist_ticket_message(ticket_id=ticket_id, message=message)
    if ticket is None:
        logger.warning("AppendMessage for unknown ticket %s", ticket_id)
        return None
    return ticket_updated_payload(ticket)
